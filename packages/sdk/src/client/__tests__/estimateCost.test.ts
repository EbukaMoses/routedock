...
const PAYEE_KP = Keypair.random()

const BASE_MANIFEST: RouteDockManifest = {
  routedock: '1.0',
...
const SESSION_MANIFEST: RouteDockManifest = {
  ...BASE_MANIFEST,
  modes: ['mpp-session'],
  pricing: {
    'mpp-session': {
...
const SIGNED_BASE = signManifest(BASE_MANIFEST, PAYEE_KP.secret())
const SIGNED_SESSION = signManifest(SESSION_MANIFEST, PAYEE_KP.secret())
...

{
  const server = await startTestServer((req, res) => {
    if (req.url === '/.well-known/routedock.json') {
      ...
      res.end(JSON.stringify(SIGNED_BASE))
    } else { res.writeHead(404); res.end() }
  })

  try {
    const client = new RouteDockClient({ wallet: Keypair.random(), network: 'testnet' })
    let x402Called = false, chargePayCalled = false
...

{
  const server = await startTestServer((req, res) => {
    if (req.url === '/.well-known/routedock.json') {
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify(SIGNED_SESSION))
    } else { res.writeHead(404); res.end() }
  })
...
// ── Test 4: invalid manifest throws ───────────────────────────────────────────

{
  ...
  try {
    ...
    try {
      ...
      threw = true
      assert.ok(err instanceof RouteDockManifestError, `expected RouteDockManifestError, got ${String(err)}`)
    }
...
{
  const server = await startTestServer((_req, res) => { res.writeHead(404); res.end() })
...

{
  ...
  try {
    ...
    const fakeResult: PaymentResult = {
      data: { ok: true }, txHash: 'deadbeef', mode: 'mpp-charge', amount: '0.0008', timestamp: Date.now(),
    }
